...
public class NextTabLayout extends HorizontalScrollView {
    ...
    private static final Pools.Pool<NextTabLayout.Tab> sTabPool = new Pools.SynchronizedPool<>(16);
    ...
    /**
     * Gravity used to fill the {@link NextTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link NextTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    public interface OnTabSelectedListener {
         ...
         */
        public void onTabSelected(NextTabLayout.Tab tab);
         ...
         */
        public void onTabUnselected(NextTabLayout.Tab tab);
         ...
         */
        public void onTabReselected(NextTabLayout.Tab tab);
    }

    private final ArrayList<NextTabLayout.Tab> mTabs = new ArrayList<>();
    private NextTabLayout.Tab mSelectedTab;

    private final NextTabLayout.SlidingTabStrip mTabStrip;
    ...
    private NextTabLayout.OnTabSelectedListener mOnTabSelectedListener;

    private NextValueAnimatorCompat mScrollAnimator;

    private ViewPager mViewPager;
    private PagerAdapter mPagerAdapter;
    private DataSetObserver mPagerAdapterObserver;
    private NextTabLayout.TabLayoutOnPageChangeListener mPageChangeListener;

    // Pool we use as a simple RecyclerBin
    private final Pools.Pool<NextTabLayout.TabView> mTabViewPool = new Pools.SimplePool<>(12);


    public NextTabLayout(Context context) {
        this(context, null);
    }

    public NextTabLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public NextTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        NextThemeUtils.checkAppCompatTheme(context);

        // Disable the Scroll Bar
        setHorizontalScrollBarEnabled(false);

        // Add the TabStrip
        mTabStrip = new NextTabLayout.SlidingTabStrip(context);
     ...
     */
    public void addTab(@NonNull NextTabLayout.Tab tab) {
     ...
     */
    public void addTab(@NonNull NextTabLayout.Tab tab, int position) {
     ...
     */
    public void addTab(@NonNull NextTabLayout.Tab tab, boolean setSelected) {
        if (tab.mParent != this) {
            throw new IllegalArgumentException("Tab belongs to a different NextTabLayout.");
        }
     ...
     * @param setSelected True if the added tab should become the selected tab.
     */
    public void addTab(@NonNull NextTabLayout.Tab tab, int position, boolean setSelected) {
        if (tab.mParent != this) {
            throw new IllegalArgumentException("Tab belongs to a different NextTabLayout.");
        }
    ...
    private void addTabFromItemView(@NonNull NextTabItem item) {
        final NextTabLayout.Tab tab = newTab();
        if (item.mText != null) {
     ...
     */
    public void setOnTabSelectedListener(NextTabLayout.OnTabSelectedListener onTabSelectedListener) {
        mOnTabSelectedListener = onTabSelectedListener;
    }

    /**
     * Create and return a new {@link NextTabLayout.Tab}. You need to manually add this using
     * {@link #addTab(NextTabLayout.Tab)} or a related method.
     *
     * @return A new Tab
     * @see #addTab(NextTabLayout.Tab)
     */
    @NonNull
    public NextTabLayout.Tab newTab() {
        NextTabLayout.Tab tab = sTabPool.acquire();
        if (tab == null) {
            tab = new NextTabLayout.Tab();
        }
        tab.mParent = this;
     ...
     */
    @Nullable
    public NextTabLayout.Tab getTabAt(int index) {
     ...
     */
    public void removeTab(NextTabLayout.Tab tab) {
        if (tab.mParent != this) {
            throw new IllegalArgumentException("Tab does not belong to this NextTabLayout.");
        }
    ...
    public void removeTabAt(int position) {
        ...
        final NextTabLayout.Tab removedTab = mTabs.remove(position);
        if (removedTab != null) {
            removedTab.reset();
    ...
    public void removeAllTabs() {
        ...
        for (final Iterator<NextTabLayout.Tab> i = mTabs.iterator(); i.hasNext(); ) {
            final NextTabLayout.Tab tab = i.next();
            i.remove();
            tab.reset();
     ...
     */
    public void setTabMode(@NextTabLayout.Mode int mode) {
        if (mode != mMode) {
            mMode = mode;
            applyModeAndGravity();
        }
    ...
    /**
     * Returns the current mode used by this {@link NextTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @NextTabLayout.Mode
     ...
     */
    public void setTabGravity(@NextTabLayout.TabGravity int gravity) {
    ...
    /**
     * The one-stop shop for setting up this {@link NextTabLayout} with a {@link ViewPager}.
     * <p>
     * <p>This method will link the given ViewPager and this NextTabLayout together so that any
    ...
    public void setupWithViewPager(@Nullable final ViewPager viewPager) {
        ...
        if (viewPager != null) {
            ...
            if (mPageChangeListener == null) {
                mPageChangeListener = new NextTabLayout.TabLayoutOnPageChangeListener(this);
            }
            mPageChangeListener.reset();
            viewPager.addOnPageChangeListener(mPageChangeListener);

            // Now we'll add a tab selected listener to set ViewPager's current item
            setOnTabSelectedListener(new NextTabLayout.ViewPagerOnTabSelectedListener(viewPager));
    ...
    /**
     * @deprecated Use {@link #setupWithViewPager(ViewPager)} to link a NextTabLayout with a ViewPager
     * together. When that method is used, the NextTabLayout will be automatically updated
    ...
    private void setPagerAdapter(@Nullable final PagerAdapter adapter, final boolean addObserver) {
        ...
        if (addObserver && adapter != null) {
            ...
            if (mPagerAdapterObserver == null) {
                mPagerAdapterObserver = new NextTabLayout.PagerAdapterObserver();
            }
    ...
    private NextTabLayout.TabView createTabView(@NonNull final NextTabLayout.Tab tab) {
        NextTabLayout.TabView tabView = mTabViewPool != null ? mTabViewPool.acquire() : null;
        if (tabView == null) {
            tabView = new NextTabLayout.TabView(getContext());
        }
        tabView.setTab(tab);
    ...
    private void configureTab(NextTabLayout.Tab tab, int position) {
    ...
    private void addTabView(NextTabLayout.Tab tab, boolean setSelected) {
        final NextTabLayout.TabView tabView = tab.mView;
    ...
    private void addTabView(NextTabLayout.Tab tab, int position, boolean setSelected) {
        final NextTabLayout.TabView tabView = tab.mView;
    ...
    private void addViewInternal(final View child) {
        if (child instanceof NextTabItem) {
            addTabFromItemView((NextTabItem) child);
        } else {
            throw new IllegalArgumentException("Only NextTabItem instances can be added to NextTabLayout");
        }
    }
    ...
    private void removeTabViewAt(int position) {
        final NextTabLayout.TabView view = (NextTabLayout.TabView) mTabStrip.getChildAt(position);
    ...
    private void animateToTab(int newPosition) {
        if (newPosition == NextTabLayout.Tab.INVALID_POSITION) {
            return;
        }
        ...
        if (startScrollX != targetScrollX) {
            if (mScrollAnimator == null) {
                mScrollAnimator = NextViewUtils.createAnimator();
                mScrollAnimator.setInterpolator(NextAnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR);
                mScrollAnimator.setDuration(ANIMATION_DURATION);
                mScrollAnimator.setUpdateListener(new NextValueAnimatorCompat.AnimatorUpdateListener() {
                    @Override
                    public void onAnimationUpdate(NextValueAnimatorCompat animator) {
    ...
    void selectTab(NextTabLayout.Tab tab) {
        selectTab(tab, true);
    }

    void selectTab(NextTabLayout.Tab tab, boolean updateIndicator) {
        if (mSelectedTab == tab) {
            ...
            if (updateIndicator) {
                final int newPosition = tab != null ? tab.getPosition() : NextTabLayout.Tab.INVALID_POSITION;
                if (newPosition != NextTabLayout.Tab.INVALID_POSITION) {
                    setSelectedTabView(newPosition);
                }
                if ((mSelectedTab == null || mSelectedTab.getPosition() == NextTabLayout.Tab.INVALID_POSITION)
                        && newPosition != NextTabLayout.Tab.INVALID_POSITION) {
    ...
    public static final class Tab {
        ...
        private NextTabLayout mParent;
        private NextTabLayout.TabView mView;
         ...
         */
        @NonNull
        public NextTabLayout.Tab setTag(@Nullable Object tag) {
            mTag = tag;
            return this;
        }
         ...
         */
        @NonNull
        public NextTabLayout.Tab setCustomView(@Nullable View view) {
            mCustomView = view;
            updateView();
            return this;
        }
         ...
         */
        @NonNull
        public NextTabLayout.Tab setCustomView(@LayoutRes int resId) {
         ...
         */
        @NonNull
        public NextTabLayout.Tab setIcon(@Nullable Drawable icon) {
            mIcon = icon;
            updateView();
            return this;
        }
         ...
         */
        @NonNull
        public NextTabLayout.Tab setIcon(@DrawableRes int resId) {
            if (mParent == null) {
                throw new IllegalArgumentException("Tab not attached to a NextTabLayout");
            }
         ...
         */
        @NonNull
        public NextTabLayout.Tab setText(@Nullable CharSequence text) {
            mText = text;
            updateView();
            return this;
        }
         ...
         */
        @NonNull
        public NextTabLayout.Tab setText(@StringRes int resId) {
            if (mParent == null) {
                throw new IllegalArgumentException("Tab not attached to a NextTabLayout");
            }
         ...
         */
        @NonNull
        public NextTabLayout.Tab setContentDescription(@StringRes int resId) {
            if (mParent == null) {
                throw new IllegalArgumentException("Tab not attached to a NextTabLayout");
            }
         ...
         */
        @NonNull
        public NextTabLayout.Tab setContentDescription(@Nullable CharSequence contentDesc) {
    ...
    class TabView extends LinearLayout implements OnLongClickListener {
        private NextTabLayout.Tab mTab;
        ...
        private void setTab(@Nullable final NextTabLayout.Tab tab) {
            if (tab != mTab) {
                mTab = tab;
                update();
            }
        ...
        final void update() {
            final NextTabLayout.Tab tab = mTab;
        ...
        public NextTabLayout.Tab getTab() {
            return mTab;
        }
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private NextValueAnimatorCompat mIndicatorAnimator;
        ...
        void animateIndicatorToPosition(final int position, int duration) {
            ...
            if (startLeft != targetLeft || startRight != targetRight) {
                NextValueAnimatorCompat animator = mIndicatorAnimator = NextViewUtils.createAnimator();
                animator.setInterpolator(NextAnimationUtils.FAST_OUT_SLOW_IN_INTERPOLATOR);
                animator.setDuration(duration);
                animator.setFloatValues(0, 1);
                animator.setUpdateListener(new NextValueAnimatorCompat.AnimatorUpdateListener() {
                    @Override
                    public void onAnimationUpdate(NextValueAnimatorCompat animator) {
                        final float fraction = animator.getAnimatedFraction();
                        setIndicatorPosition(
                                NextAnimationUtils.lerp(startLeft, targetLeft, fraction),
                                NextAnimationUtils.lerp(startRight, targetRight, fraction));
                    }
                });
                animator.setListener(new NextValueAnimatorCompat.AnimatorListenerAdapter() {
                    @Override
    ...
    private int getDefaultHeight() {
        ...
        for (int i = 0, count = mTabs.size(); i < count; i++) {
            NextTabLayout.Tab tab = mTabs.get(i);
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        ...
        // We return the default layout params so that we don't blow up if we're given a NextTabItem
     ...
     * necessary calls back to the provided {@link NextTabLayout} so that the tab position is
     * kept in sync.
     * <p>
     * <p>This class stores the provided NextTabLayout weakly, meaning that you can use
     * {@link ViewPager#addOnPageChangeListener(ViewPager.OnPageChangeListener)
     * addOnPageChangeListener(OnPageChangeListener)} without removing the listener and
     * not cause a leak.
     */
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<NextTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(NextTabLayout NextTabLayout) {
            mTabLayoutRef = new WeakReference<>(NextTabLayout);
        }
        ...
        public void onPageScrolled(int position, float positionOffset,
            ...
            final NextTabLayout NextTabLayout = mTabLayoutRef.get();
            if (NextTabLayout != null) {
                ...
                NextTabLayout.setScrollPosition(position, positionOffset, updateText, updateIndicator);
            }
        }
        ...
        public void onPageSelected(int position) {
            final NextTabLayout NextTabLayout = mTabLayoutRef.get();
            if (NextTabLayout != null && NextTabLayout.getSelectedTabPosition() != position) {
                // Select the tab, only updating the indicator if we're not being dragged/settled
                // (since onPageScrolled will handle that).
                final boolean updateIndicator = mScrollState == SCROLL_STATE_IDLE
                        || (mScrollState == SCROLL_STATE_SETTLING
                        && mPreviousScrollState == SCROLL_STATE_IDLE);
                NextTabLayout.selectTab(NextTabLayout.getTabAt(position), updateIndicator);
            }
        }
    ...
    /**
     * A {@link NextTabLayout.OnTabSelectedListener} class which contains the necessary calls back
     * to the provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements NextTabLayout.OnTabSelectedListener {
